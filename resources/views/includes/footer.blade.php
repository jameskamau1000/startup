          ...
          <li><a class="link-footer" href="{{ url('campaigns/featured') }}">{{ trans('misc.featured_campaign') }}</a></li><li>
            ...
            @endguest
              ...
              <div class="dropdown-menu dropdown-menu-macos">
                @foreach (App\Models\Languages::orderBy('name')->get() as $languages)
                  <a @if ($languages->abbreviation != config('app.locale')) href="{{ url('change/lang', $languages->abbreviation) }}" @endif class="dropdown-item dropdown-lang mb-1 @if( $languages->abbreviation == config('app.locale') ) active text-white @endif">
    ...
    <div class="col-md-3">
      ...
      <div id="installContainer" class="d-none-custom">
        <button class="btn btn-outline-light w-100 mb-4 mt-3" id="butInstall" type="button">
          <i class="bi-phone me-1"></i> {{ __('misc.install_web_app') }}
        </button>
      </div>
    ...
    <div class="row">
      ...
      <div class="col-md-6 text-right social-links">
        ...
        <ul class="list-inline float-right list-social">
          ...
          @if( $settings->twitter != '' )
            <li class="list-inline-item"><a href="{{$settings->twitter}}" class="ico-social"><i class="bi-twitter-x"></i></a></li>
          @endif
