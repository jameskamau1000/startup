            ...
            @endif
               ...
               @if ($data->total() !=  0 && $data->count() != 0)
                 ...
                 @foreach ($data as $user)
					...
					<form method="POST" action="{{ route('user.destroy', $user->id) }}" id="form{{ $user->id }}" class="d-inline">
						@csrf
						@method('DELETE')
						<button type="submit" data-url="{{ $user->id }}" class="btn btn-danger rounded-pill btn-sm e-none actionDelete">
							<i class="bi-trash3"></i>
						</button>
					</form>
				 </div>
