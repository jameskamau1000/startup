              ...
              @if ($data->count() !=  0)
                ...
                @foreach( $data as $lang )
                  <tr>
                   ...
                   @if ($data->count() != 1)
                  <form method="POST" action="{{ route('languages.destroy', $lang->id) }}" id="form{{ $lang->id }}"
                    class="d-inline-block align-top">
                    @csrf
                    @method('DELETE')
                    <button type="submit" data-url="{{ $lang->id }}" class="btn btn-danger rounded-pill btn-sm e-none actionDelete">
                      <i class="bi-trash3"></i>
                    </button>
                  </form>
                    @endif
