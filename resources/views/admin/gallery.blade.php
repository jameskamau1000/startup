               ...
               @if ($data->count() !=  0)
                  <tr>
									 ...
									 @foreach ($data as $gallery)
										...
										<form method="POST" action="{{ url('panel/admin/gallery/delete', $gallery->id) }}" id="form{{ $gallery->id }}" class="displayInline">
											@csrf
										
											<button type="submit" class="btn btn-danger btn-sm padding-btn actionDelete" data-url="{{ $gallery->id }}">
												{{ __('admin.delete') }}
											</button>
										</form>
													 </td>
