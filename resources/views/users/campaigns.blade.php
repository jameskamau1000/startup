  ...
  <div class="table-responsive">
   <table class="table m-0">
        ...
        @foreach($data as $campaign )
          ...
          @if ($funds != 0)
            <form action="{{ url('campaign/withdrawal', $campaign->id) }}" method="POST" class="displayInline">
              @csrf
              <button type="submit" class="btn btn-success btn-sm padding-btn no-hover">{{ __('misc.make_withdrawal') }}</button>
            </form>
